...
module Exports
  ...
  class PdfGenerator
    ...
    def render_header(pdf)
      pdf.font_size(20) { pdf.text clean(@assessment.name), style: :bold }
      pdf.move_down 4
    ...
    def render_skill_card(pdf, skill)
      ...
      pdf.font_size(11) do
        pdf.text clean(skill.skill_label), style: :bold
        ...
        pdf.text clean(level_text)
      end

      pdf.move_down 4

      if skill.competency_summary.present?
        pdf.font_size(10) { pdf.text clean(skill.competency_summary) }
      end

      if skill.evidence.is_a?(Array) && skill.evidence.any?
        pdf.move_down 4
        pdf.font_size(10) do
          pdf.text "Evidence:", style: :bold
          skill.evidence.each { |quote| pdf.text "  * #{clean(quote)}" }
        end
      end
      ...
      if override&.assessor_notes.present?
        ...
        pdf.font_size(10) do
          ...
          pdf.text "  #{clean(override.assessor_notes)}"
        end
      end
    ...
    def render_fit_gap_section(pdf)
      ...
      pdf.font_size(16) { pdf.text "Fit/Gap Analysis - #{clean(@vacancy.role_title)}", style: :bold }
      pdf.move_down 8
      ...
      comparisons.each do |c|
        table_data << [
          ...
          clean(RESULT_LABELS[c['result']] || c['result']),
          c['delta'] ? (c['delta'].to_i > 0 ? "+#{c['delta']}" : c['delta'].to_s) : '-'
        ]
      end
      ...
      if @fit_gap.culture_narrative.present?
        ...
        pdf.move_down 4
        pdf.font_size(10) { pdf.text clean(@fit_gap.culture_narrative) }
      end
      ...
      if @fit_gap.overall_narrative.present?
        ...
        pdf.move_down 4
        pdf.font_size(10) { pdf.text clean(@fit_gap.overall_narrative) }
      end
    end
